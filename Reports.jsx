...
export default function Reports({ employees }) {
  ...
  const empStats = Object.entries(empMap)
    .map(([id, v]) => ({ id, ...v, color: employees.find(e => e.id === id)?.color || 'var(--pink)' }))
    ...
    <div className="page">
      <div className="ph">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {RANGES.map(r => (
            <button key={r} className={`btn ${range===r ? 'btn-p' : 'btn-g'} btn-sm`} onClick={() => setRange(r)}>{r}</button>
          ))}
        </div>
      </div>
        ...
        : <>
          <div className="sg">
            {[
              ...
              <div className="sc" key={s.l}><div className="sl">{s.l}</div><div className="sv" style={{ color: s.c }}>{s.v}</div></div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div className="card">
              ...
              {!empStats.length && <div className="empty">No data for this period</div>}
              <table>
                    ...
                    <tr key={e.id}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                          <span className="ea" style={{ background: e.color, width: 26, height: 26, fontSize: 10 }}>{(e.name||'?')[0]}</span>
                          {e.name}
                        </div>
                      </td>
            ...
            <div className="card">
              ...
              {!tickets.length && <div className="empty">No tickets for this period</div>}
              <table>
