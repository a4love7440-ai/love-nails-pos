...
export default function Employees({ employees, setEmployees, showToast }) {
  ...
  return (
    <div className="page">
      <div className="ph">
        <div>
          <div className="ptitle">Employees</div>
          <div className="psub">{employees.filter(e => e.active).length} active · {employees.length} total</div>
        </div>
              ...
              <tr key={emp.id}>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                    <span className="ea" style={{ background: emp.color || 'var(--pink)' }}>{emp.name[0]}</span>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 7 }}>
                    <button className="btn btn-g btn-sm" onClick={() => openEdit(emp)}><Ico name="edit" size={13} /></button>
                    <button className="btn btn-g btn-sm" onClick={() => toggle(emp)}>{emp.active ? 'Deactivate' : 'Activate'}</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="fr">
              ...
              <div className="fg"><label>Commission %</label><input placeholder="e.g. 60" value={form.commission_rate} onChange={e => setForm({ ...form, commission_rate: e.target.value })} /></div>
              <div className="fg">
