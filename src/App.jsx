  ...
  return (
    <div className="app">
      <aside className="sidebar">
        <div className="sb-logo"><h1>Love Nails</h1><span>POS System</span></div>
        <nav className="nav">
          {NAV.map(n => (
        ...
        <div className="sb-foot"><div>MVP v1.0</div><div style={{ marginTop: 3 }}>{todayStr()}</div></div>
      </aside>
      <main className="main">
        {page === 'dashboard'    && <Dashboard employees={employees} />}
        {page === 'ticket'       && <NewTicket employees={employees} customers={customers} showToast={showToast} onDone={() => setPage('dashboard')} />}
