...
export default function NewTicket({ employees, customers, showToast, onDone }) {
  ...
  async function handleComplete() {
    ...
    try {
      ...
      const { data: ticket, error: tErr } = await supabase.from('tickets').insert({ customer_name: customerName, customer_id: selCustomer || null, subtotal, discount_amt: discAmt, tip_total: tipTotal, total, pay_method: payMethod, status: 'Completed' }).select().single()
      if (tErr) throw tErr
      const itemRows = items.map(it => ({ ticket_id: ticket.id, service_name: it.name, price: it.price, employee_id: it.employeeId || null, employee_name: employees.find(e => e.id === it.employeeId)?.name || null }))
      ...
      const tipRows = Object.entries(tipInputs).filter(([, v]) => parseFloat(v) > 0).map(([empId, v]) => ({ ticket_id: ticket.id, employee_id: empId, employee_name: employees.find(e => e.id === empId)?.name || null, amount: parseFloat(v) }))
  ...
  return (
    <div className="page-wide">
      ...
      <div className="psub">Select services → assign staff → complete</div>
      <div className="tl">
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', gap: 14 }}>
              <div style={{ flex: 1 }}><label>Customer</label><select value={selCustomer} onChange={e => setSelCustomer(e.target.value)}><option value="">Walk-in</option>{customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
              {!selCustomer && <div style={{ flex: 1 }}><label>Walk-in name</label><input placeholder="Optional" value={walkIn} onChange={e => setWalkIn(e.target.value)} /></div>}
            </div>
          </div>
          <div className="card">
            <div className="cf">
              {[{ id: 'all', name: 'All', color: 'var(--pink)' }, ...CATEGORIES].map(c => (
                <button key={c.id} className={`cb ${selCat === c.id ? 'on' : ''}`} style={selCat === c.id ? { background: c.color, borderColor: c.color } : {}} onClick={() => setSelCat(c.id)}>{c.name}</button>
              ))}
            </div>
            <div className="svcg">
              {filtered.map(svc => (
        ...
        <div className="bp">
            ...
            return (
              <div key={i} className="bi">
                <div className="bii">
                  ...
                  <div className="bie">{emp ? <><span className="ea" style={{ width: 16, height: 16, fontSize: 9, background: emp.color }}>{emp.name[0]}</span>{emp.name}</> : <span style={{ color: 'var(--red)', cursor: 'pointer' }} onClick={() => setAssignIdx(i)}>⚠ Assign staff</span>}</div>
                </div>
            ...
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
              ...
              {empWithSvc.map(emp => (
                <div key={emp.id} style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
                  ...
                  <input placeholder="$0" style={{ width: 80 }} value={tipInputs[emp.id] || ''} onChange={e => setTipInputs(p => ({ ...p, [emp.id]: e.target.value }))} />
                </div>
              ))}
            </div>
          )}
          <div className="btot">
            ...
            <div className="br"><span>Discount</span><input placeholder="0" style={{ width: 80, textAlign: 'right' }} value={discount} onChange={e => setDiscount(e.target.value)} /></div>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div className="pg">{PAYMENT_METHODS.map(m => <button key={m} className={`pb ${payMethod === m ? 'sel' : ''}`} onClick={() => setPayMethod(m)}>{m}</button>)}</div>
          </div>
          <button className="btn btn-p" style={{ width: '100%', marginTop: 18, justifyContent: 'center', padding: '13px' }} disabled={saving || !items.length} onClick={handleComplete}>
      ...
      {assignIdx !== null && (
        <div className="overlay" onClick={() => setAssignIdx(null)}>
          <div className="modal" style={{ maxWidth: 380 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 18 }}>For: <strong>{items[assignIdx]?.name}</strong></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {activeEmps.map(emp => <button key={emp.id} className="btn btn-g" style={{ justifyContent: 'flex-start', gap: 10 }} onClick={() => { setItems(p => p.map((it, i) => i === assignIdx ? { ...it, employeeId: emp.id } : it)); setAssignIdx(null) }}><span className="ea" style={{ background: emp.color, width: 28, height: 28, fontSize: 11 }}>{emp.name[0]}</span>{emp.name}</button>)}
            </div>
            <div className="mf"><button className="btn btn-g btn-sm" onClick={() => setAssignIdx(null)}>Skip</button></div>
          </div>
        </div>
      )}
    </div>
