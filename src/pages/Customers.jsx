...
export default function Customers({ showToast }) {
  ...
  function openNew() { setEditing(null); setForm({ name:'', phone:'', email:'', notes:'', allergies:'' }); setShowModal(true) }
  function openEdit(c) { setEditing(c); setForm({ name: c.name, phone: c.phone||'', email: c.email||'', notes: c.notes||'', allergies: c.allergies||'' }); setShowModal(true) }
  ...
  return (
    <div className="page">
      ...
      <div style={{ marginBottom: 16 }}><input placeholder="Search by name or phone…" value={search} onChange={e => setSearch(e.target.value)} /></div>
      <div className="card">
        <table>
          <thead><tr><th>Name</th><th>Phone</th><th>Notes</th><th>Allergies</th><th></th></tr></thead>
          <tbody>
            ...
            {filtered.map(c => (
              <tr key={c.id}>
                ...
                <td><div style={{ display: 'flex', gap: 7 }}><button className="btn btn-g btn-sm" onClick={() => openEdit(c)}><Ico name="edit" size={13} /></button><button className="btn btn-d btn-sm" onClick={() => del(c.id)}><Ico name="trash" size={13} /></button></div></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="fr">
              ...
              <div className="fg full"><label>Allergies ⚠️</label><input value={form.allergies} onChange={e => setForm({ ...form, allergies: e.target.value })} /></div>
            </div>
            <div className="mf">
