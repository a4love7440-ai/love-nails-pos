...
export default function Appointments({ employees, showToast }) {
  ...
  async function load() {
    ...
    const { data } = await supabase.from('appointments').select('*').gte('created_at', today.toISOString()).order('time', { ascending: true })
    setAppts(data || [])
    setLoading(false)
  }
        ...
        return (
          <div key={a.id} className="ac">
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2 }}>{a.service || '—'}{emp ? ` · ${emp.name}` : ''}</div>
            ...
            <select style={{ width: 140, fontSize: 12, padding: '6px 9px' }} value={a.status} onChange={e => updateStatus(a.id, e.target.value)}>
      ...
      {showModal && (
        <div className="overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="fr">
              <div className="fg"><label>Time *</label><input placeholder="10:00 AM" value={form.time} onChange={e => setForm({ ...form, time: e.target.value })} /></div>
              <div className="fg"><label>Customer Name *</label><input placeholder="Name" value={form.customer_name} onChange={e => setForm({ ...form, customer_name: e.target.value })} /></div>
              <div className="fg"><label>Service</label><select value={form.service} onChange={e => setForm({ ...form, service: e.target.value })}><option value="">Select service</option>{SERVICES.map(s => <option key={s.id}>{s.name}</option>)}</select></div>
              <div className="fg"><label>Employee</label><select value={form.employee_id} onChange={e => setForm({ ...form, employee_id: e.target.value })}><option value="">Any available</option>{employees.filter(e => e.active).map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
              <div className="fg full"><label>Notes</label><textarea rows={2} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} /></div>
            </div>
            <div className="mf">
